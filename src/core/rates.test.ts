...
describe("netApyWad (§5.2 leveraged-restake)", () => {
  it("unlevered (b=0) equals collateral compound", () => {
    const net = netApyWad(WAD, 0n, 0n, pctWad(3), pctWad(2), pctWad(2.5));
    // (1.03)(1.02) - 1 = 0.0506
  ...
  it("exact fixture: b=0.5, stake 3%, supply 2%, debt 2.5% → 0.0634e18", () => {
    ...
    expect(netApyWad(WAD, 0n, WAD / 2n, pctWad(3), pctWad(2), pctWad(2.5))).toBe(63_400_000_000_000_000n);
  });
  it("leverage amplifies a positive carry", () => {
    const unlev = netApyWad(WAD, 0n, 0n, pctWad(3), pctWad(2), pctWad(2.5));
    const lev = netApyWad(WAD, 0n, WAD / 2n, pctWad(3), pctWad(2), pctWad(2.5));
  ...
  it("negative carry makes leverage hurt", () => {
    ...
    const lev = netApyWad(WAD, 0n, WAD / 2n, pctWad(1), pctWad(1), pctWad(8));
    const unlev = netApyWad(WAD, 0n, 0n, pctWad(1), pctWad(1), pctWad(8));
