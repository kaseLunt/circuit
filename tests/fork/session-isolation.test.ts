 ...
 * with test doubles.
 *
 * The per-session anvils fork FROM the suite's pinned anvil (`ANVIL_URL`) at the pinned
 * block: the suite anvil serves its fork backend's state at that height regardless of
 * what later suites mined on it, the block hash at the pin is still mainnet's, and the
 * upstream cache stays warm — the same reasoning as `ANVIL_EXTERNAL` in global-setup.
 *
