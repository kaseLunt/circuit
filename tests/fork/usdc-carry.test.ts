 ...
 * historical-state wedge): a DEDICATED pristine upstream at the pin, spawned here, whose head
 * never moves. Ports are clear of the base anvil (8547), the production default range
 * (9545+), the isolation suite (9640–9650) and the drills suite (9653–9659).
 */
...
import { hexQuantity, hexWord, record } from "./harness";

const PINNED_HASH = readsMeta.pinned_block.hash as Hex;

const UPSTREAM_PORT = 9663;
...
describe("W09 fork gate — the USDC carry on the real session composition", () => {
  ...
  beforeAll(async () => {
    ...
    if (forkUrl === undefined || forkUrl === "") {
      throw new Error("FORK_RPC_URL is required — the dedicated carry upstream forks from it");
    }
    ...
    const logStream = createWriteStream(join(tmpdir(), "circuit-carry-upstream.log"), {
      flags: "w",
    });
    let stderrTail = "";
    ...
    upstream.on("error", (e) => {
      upstreamFailure = new Error(`failed to spawn dedicated carry upstream anvil: ${e.message}`);
    });
    upstream.on("exit", (code) => {
      if (upstreamFailure === null && !upstreamTearingDown) {
        upstreamFailure = new Error(
          `dedicated carry upstream anvil exited (code ${code}): ${stderrTail}`,
        );
      }
    });

    await pollUntilReady({
      what: `dedicated carry upstream readiness at ${UPSTREAM_URL}`,
      ...
      onTimeout: () =>
        new Error(`dedicated carry upstream not ready after ${UPSTREAM_READY_BUDGET_MS}ms: ${stderrTail}`),
    });

    const head = await blockNumberAt(UPSTREAM_URL);
    if (head !== PINNED_BLOCK) {
      throw new Error(
        `dedicated carry upstream head is ${head}, not the pin ${PINNED_BLOCK} — nothing may mine on it`,
      );
    }
    ...
    if (pinned === null || pinned.hash !== PINNED_HASH) {
      throw new Error(
        `dedicated carry upstream identity mismatch at ${PINNED_BLOCK}: ${pinned?.hash ?? "null"}`,
      );
    }
    record(`dedicated carry upstream ready at ${UPSTREAM_URL}, pinned to ${PINNED_BLOCK}`);
  });
  ...
  afterAll(async () => {
    ...
    if (upstreamTracker !== null && !upstreamTearingDown) {
      ...
      if (head !== null && head !== PINNED_BLOCK) {
        throw new Error(`carry upstream head moved to ${head} — a drill leaked onto the upstream`);
      }
    }
